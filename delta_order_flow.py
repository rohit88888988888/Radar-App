...
inside the LIVE WS PUSH message specifically. What IS now confirmed (via
docs.delta.exchange's own REST sample response) is the field name and
values for trade side generally: {"side": "buy", "size": 0, "price":
"string", "timestamp": 0} — lowercase "buy" / "sell" strings, field name
"side". Delta's WS trades channel is documented to carry the same trade
object shape as this REST endpoint, so _extract_side()'s primary check
(trade.get("side")) is now backed by their docs, not just a guess — the
raw-message watchdog below stays as a belt-and-braces check in case the
live push envelope nests the trade object differently than the REST list.

TWO MORE ASSUMPTIONS THAT ONLY A LIVE CONNECTION CAN CONFIRM (flagged
here rather than silently assumed):
  1. Keep-alive: this uses websocket-client's protocol-level ping/pong
     frames (ping_interval=25s). If Delta instead expects an application-
     level JSON ping message on the wire, protocol pings may not satisfy
     it and the server could close the connection — the reconnect loop
     will recover either way, but if you see repeated close/reconnect
     cycles every ~25-30s in the logs, this is the first thing to check.
  2. Handshake headers: no custom Origin/User-Agent header is sent. If
     Delta's WS gateway rejects connections without one, you'd see the
     handshake fail immediately (visible as a connection error in the
     logs before any "connected" line) rather than the null-data case
     this file is otherwise built to diagnose.
...
_trade_buffer = defaultdict(lambda: deque(maxlen=10000))  # symbol -> deque of
    # (timestamp, side, size, price), capped defensively. Normally get_order_flow()
    # trims old entries by time on every call, but this cap guards against
    # unbounded memory growth if a symbol is subscribed but never queried.
_ws_connected = False
_raw_message_logged = False  # log the very first raw message once, for verification
_subscribed_symbols = []
_started = False  # guards against start() being called twice — see start()
_watchdog_scheduled = False  # only ever schedule the no-data watchdog once total,
    # not once per reconnect (see _on_open) — otherwise a flaky connection that
    # reconnects 10 times would stack 10 redundant timers checking the same thing.
...
def _extract_side(trade):
    """Trade side field name/values are confirmed for Delta's trade objects
    generally ("side": "buy"/"sell", lowercase) — see module docstring.
    Lowercased here defensively in case the live WS push ever differs in
    case from the documented REST shape. Fallback keys cover exchanges/
    edge cases where "side" might be renamed."""
    raw = trade.get("side") or trade.get("buyer_role") or trade.get("taker_side")
...
def _on_open(ws, symbols):
    global _ws_connected, _watchdog_scheduled
    _ws_connected = True
    ...
    try:
        ...
        # send() can throw before on_error/on_close even fire. Don't let
        # that exception escape into websocket-client's internal dispatch —
        # log it and let the normal reconnect loop in _run_forever recover.
        log.error(f"❌ order_flow: failed to send subscribe payload: {e}")
        return
    if not _watchdog_scheduled:
        _watchdog_scheduled = True
...
def _run_forever(symbols):
    ...
    max_delay = 60
    while True:
        attempt_connected = {"ok": False}  # local per-attempt flag, not the shared
            # global — avoids depending on _ws_connected/_on_close firing cleanly
            # in every crash path just to decide whether to reset backoff.

        def _mark_open(w):
            attempt_connected["ok"] = True
            _on_open(w, symbols)
        ...
        # Exponential backoff, capped at max_delay — a real outage or a wrong
        # URL/region shouldn't turn into a tight loop hammering Delta's servers
        # every 5s indefinitely. Resets to the base delay the moment this
        # specific attempt actually reached on_open, regardless of how it died.
        delay = RECONNECT_DELAY_SECONDS if attempt_connected["ok"] else min(delay * 2, max_delay)
        log.warning(f"⚠️ order_flow: reconnecting in {delay}s...")
        time.sleep(delay)
...
def start(symbols):
    ...
    Safe to call more than once by accident: a second call is a no-op. Without
    this guard, two live WebSocket threads would both append every trade to
    the same buffer, silently doubling every volume number get_order_flow()
    reports — a bug that's easy to trigger by e.g. importing this module from
    two different entry points, or a start() call surviving a code path that
    runs twice."""
    global _subscribed_symbols, _started
    with _lock:
        if _started:
            log.warning("⚠️ order_flow: start() called again — ignoring, "
                        "already running (would otherwise double-count every trade).")
            return None
        _started = True
