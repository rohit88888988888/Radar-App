...
# gunicorn imports this file as a module and never runs the __main__ block, so
# putting start() there would mean the WebSocket thread never launches under
...
def flow():
    try:
        ...
        # Returned as JSON (not a Flask 500 HTML page) so the dashboard's
        # fetch().json() in the browser doesn't itself throw and blank the page.
...
if __name__ == "__main__":
    # PORT comes from Railway's environment at runtime — it assigns a
    # different port per deploy and only routes traffic to that port.
    # Hardcoding 8081 here was why Railway showed "Unexposed service" and
    # the public domain gave DNS_PROBE_FINISHED_NXDOMAIN: nothing was
    # listening where Railway expected. 8081 stays only as a local-dev
